...
onready var _jump_velocity : float = 0.0
onready var _jump_gravity : float = 0.0
onready var _fall_gravity : float = 0.0
...
func jump(_jump_height, _jump_time_to_peak, _jump_time_to_descent):
	if _is_ground:
		print("icon_jump")
		_jump_velocity = ((2.0 * _jump_height) / _jump_time_to_peak) * -1.0
		_jump_gravity = ((-2.0 * _jump_height) / (_jump_time_to_peak * _jump_time_to_peak)) * -1.0
		_fall_gravity= ((-2.0 * _jump_height) / (_jump_time_to_descent * _jump_time_to_descent)) * -1.0
		_velocity.y = _jump_velocity
		_is_ground = false

func floated(_jump_height, _jump_time_to_peak, _jump_time_to_descent):
	_jump_velocity = ((2.0 * _jump_height) / _jump_time_to_peak) * -1.0
	_jump_gravity = ((-2.0 * _jump_height) / (_jump_time_to_peak * _jump_time_to_peak)) * -1.0
	_fall_gravity= ((-2.0 * _jump_height) / (_jump_time_to_descent * _jump_time_to_descent)) * -1.0
	_velocity.y = _jump_velocity
...
#	_jump_height = value
#	print("change jump height " + str(_jump_height))
#
